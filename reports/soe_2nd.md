...
2. **OCR PN reconciliation (OCR only; Round 2 + 2b, Auditor rule).** Collect every reading of the 10-digit PN stem in the scan: invoice item row (dotted Bosch PN), Customer PN column (no-dot form, e.g. `026501109757G`), and cargo list / transport order / delivery note PN fields. A reading only counts when the full stem directly precedes the 3-char suffix, and lines whose text before the PN is a numeric label (Volume/cdm/weight/kg/Sum) are skipped, so `26) Volume in cdm 576` can never be a candidate. Then:
   - (a) All readings agree → keep.
   - (b) Group readings into look-alike classes (G/6, S/5, B/8, O/0, Z/2, I/1). The dominant class must hold a **strict majority of all readings**; otherwise `items.part_no` becomes needs_gold. Readings outside it are ignored as outliers (e.g. `-876` on 1267475174).
   - (c) Inside that class, decide **per character position**. **G/6 → G, even when G is the minority.** Tesseract's G→6 error is one-directional on these prints: 7405713 reads -57G×1 vs -576×3 and prints -57G. Every other pair (0/O, 1/I, 5/S, 2/Z, 8/B) flips both ways, so the character (digit or letter) that holds a **strict majority of the class members** at that position wins. A tie gives needs_gold. So a real `-2U1` read 2U1×3 / ZU1×1 stays `-2U1`, `-501`×2 / `-S01`×1 stays `-501`, and 2U1×2 / ZU1×2 is needs_gold.
   - (d) A separator misread with no second reading to confirm it → needs_gold. Text-layer PDFs are never touched.
   - Round 2 → 2b correction: the @06ff88b code required a strict majority only for the *class* and then always preferred the letter form inside it. That is what flipped 7405713 (57G×1 vs 576×3). It would also have flipped a real digit suffix to a letter look-alike. 2b limits letter priority to G/6.
   - Why not 'Customer PN first': on 7369301 and 7405713 Tesseract also reads the printed Customer PN `026501109757G` as `0265011097576` (targeted re-OCR of that crop at 300/400/600 dpi still gives `…576`), so the OCR'd Customer PN can't break the tie.
   - Pipeline candidates: 7369301 -57G×2, -576×2. 7405713 -57G×1, -576×3. 1267475174 -57G×4, -576×2, -876×1 (outlier). 1267502206 -57G only. All three differ only at the G/6 position, so all give `-57G`.
